...
Actual data: http://py4e-data.dr-chuck.net/regex_sum_1447193.txt 
(There are 82 values and the sum ends with 107)
...
# There can be any number of numbers in each line
# (including none).

"""Handling The Data

The basic outline of this problem is to read the file, 
look for integers using the re.findall(), looking for 
a regular expression of '[0-9]+' and then converting 
the extracted strings to integers and summing up the 
integers.
...
for line in fh:
    line = re.findall(
        "[0-9]+", line
    )  # finds all numbers '.[0-9]*[0-9]' was before and it missed py4e.com and etx
    for number in line:
